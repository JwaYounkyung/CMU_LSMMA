...
def load_audio(audio_path, sample_rate=32000, mono=True):
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='snf3/',
    ...
    parser.add_argument("--feat_layer",type=str,default="conv6",
...
def extract_features(args):
    ...
    state_dict = torch.load('weights/esc50-passt-s-n-f128-p16-s10-fold1-acc.967.pt')
    ...
    for sound_sample, audio_path in gen_audio_from_dir(args.input_dir,
        ...
        new_sample = new_sample.squeeze().unsqueeze(0)
        feats = model.forward(new_sample)
        sel_feats = feats.squeeze().cpu().data.numpy()
